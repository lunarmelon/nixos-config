...
select_interface() {
  chosen=$( (
    ...
    echo "$icon_close"
  ) | display_menu 1 "$tr_select_interface_prompt")
...
menu_addresses() {
  ...
  while true; do
    ...
    chosen="$( (
      for i in "${addresses_list[@]}"; do echo "$i"; done
      echo "$icon_close"
...
menu_dns() {
  ...
  while true; do
    ...
    chosen="$( (
      for i in "${dns_list[@]}"; do echo "$i"; done
      echo "$icon_close"
...
menu_known_connections() {
  ...
  while true; do
    ...
    chosen=$( (
      for i in "${profiles_list[@]}"; do echo -e "$i" | cut --delimiter $'\0' --fields 2; done
      echo "$icon_close"
    ) | display_menu 1 "$tr_known_connections_message")
...
connect_wifi() {
  ...
  if [[ "$wifi_security" =~ ^(${icon_wifi_1}|${icon_wifi_2}|${icon_wifi_3}) ]]; then
    ...
    # 4. Connect WITHOUT a password argument for open networks
...
menu_submenu() {
  while true; do
    ...
    "$icon_wifi_disable"*)
      ...
      send_notification 1 1 "$tr_notice_disable_wifi_summary" "$tr_notice_disable_wifi_body"
      exit 0
      ;;
...
menu_main() {
  ...
  while true; do
    ...
    "$icon_wifi_enable"*)
      nmcli radio wifi on
      send_notification 1 1 "$tr_notice_enable_wifi_summary" "$tr_notice_enable_wifi_body"
      exit 0
      ;;
    "$icon_wifi_disable"*)
      nmcli radio wifi off
...
while [ -n "$1" ]; do
  ...
  '--custom')
    ...
    ;;
  *)
    echo "$program_name: Wrong option given. See usage with -h or --help." >&2
    exit 1
    ;;
  esac
  shift
